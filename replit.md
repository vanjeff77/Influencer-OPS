...
- **Email Communication**:
    ...
    - **Bulk Email Sending**: Queue-based 1:1 delivery with throttling, WYSIWYG editor, variable substitution. Uses Gmail API when `useGmailApi` is enabled on the email account, otherwise SMTP. Gmail API path uses `forceUniqueThread` (unique Message-ID + X-Entity-Ref-ID) to prevent thread merging. Rate limit errors (`429`, `rateLimitExceeded`, `quotaExceeded`, `RESOURCE_EXHAUSTED`) are treated as transient and retried.
    ...
    - **Auto Email Sync**: Gmail History API-based incremental sync with background worker and frontend auto-refresh. IMAP auto-sync via `fetchInboxReplies` (two-phase: headers-first matching, then full body fetch for matches only). 60-second sync interval with 45-second IMAP timeout. Supports both Gmail OAuth and IMAP provider accounts. Both Gmail and IMAP sync paths update `subjectPrefix` from inbound message subject (stripping `Re:` prefix) for accurate reply threading. Direction detection uses workspace email domains (not just sync account email) — `isTeamEmail()` checks sender against all team member domains via `getWorkspaceEmailDomains()` (cached 5min TTL, excludes common consumer domains).
...
- **Slack Bot for Email Notifications**:
    ...
    - **AI Draft Actions**: "✏️ 초안 사용하기" opens modal with editable text + To/CC display → "📤 발송하기" submits. Regenerate with feedback (modal), alternative classifications, dismiss. Progress indicator during generation. Modal uses open-first-update-later pattern: loading modal opens immediately via `views.open` (preserves trigger_id), then `views.update` populates data after DB queries. `block_actions` responses sent immediately (200) with background processing.
    - **Slack Interaction Pattern**: `/api/slack/interactions` responds with 200 immediately for `block_actions` to avoid Slack's 3-second timeout; handlers run in background. `view_submission` still awaited for validation error responses.
